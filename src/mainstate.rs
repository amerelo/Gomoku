...
impl MainState 
{
	pub fn new(ctx: &mut Context) -> GameResult<MainState>
	{
		...
		Ok(MainState {
			background, tile, goban, goban_pos, text, torch, torch_p, static_light,
...
impl event::EventHandler for MainState {
	...
	fn draw(&mut self, ctx: &mut Context) -> GameResult<()>
	{
		...
		let origin = DrawParam {
			dest: Point2::new(0.0, 0.0),
			scale: Point2::new(1.5, 1.5),
			..Default::default()
		};

		let origin_deux = DrawParam {
			dest: Point2::new(self.goban_pos[0] as f32, self.goban_pos[1] as f32),
		...
		let y_start: f32 = self.goban_pos[0] as f32 + 13.0;
		let y_dif: f32 = 46.0;
		let x_start: f32 = self.goban_pos[1] as f32 + 15.0;
		
		// for line in 0..19 {
		// 	let x_start: f32 = self.goban_pos[1] as f32 + 15.0 + y_dif * line as f32;
			...
			for elem in 0..19 {
				...
				graphics::draw_ex(
					...
					&self.tile,
					DrawParam {
						dest: Point2::new(new_pos, x_start),
		...
		self.render_light(ctx, torch, origin_shadow, canvas_origin)?;
		self.render_light(ctx, torch_p, origin_shadow, canvas_origin)?;
		// self.render_light(ctx, light, origin_shadow, canvas_origin)?;
		...
		graphics::draw_ex(ctx, &self.shadows, origin_shadow)?;
		graphics::draw_ex(ctx, &self.lights, origin_shadow)?;
		...
		graphics::draw_ex(ctx, &self.foreground, origin_shadow)?;
