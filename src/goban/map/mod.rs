...
impl Map
{
            ...
            if *elem_y != 0
            {
                for y in 0..(SIZEMAP)
                {
                    ...
                    if value == 0o11111 && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y)), &Player::One)
                                        && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 1)), &Player::One)
                                        && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 2)), &Player::One)
                                        && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 3)), &Player::One)
                                        && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 4)), &Player::One)
                    {
                        self.is_finish = Finish::AlignPlayerOne;
                        return ;
                    }
                    else if value == 0o22222 && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y)), &Player::Two)
                                             && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 1)), &Player::Two)
                                             && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 2)), &Player::Two)
                                             && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 3)), &Player::Two)
                                             && !self.is_capturable(self.all_conv_xy((x as i128, RSIZEMAP - y + 4)), &Player::Two)
                    {
                        self.is_finish = Finish::AlignPlayerTwo;
                        return ;
                    }
    ...
    fn three_move_number(&self, (x, y):(i128, i128), (slot_hv, slot_d): ([(i128, i128, i128); 9], [(i128, i128, i128); 9])) -> usize
    {
        ...
        count += match x
        {
            ...
            16 => slot_cmp![self.value[y as usize], (RSIZEMAP - x) * 3 ; slot_hv; [0, 1, 3, 4, 6]] as usize,
            15 => slot_cmp![self.value[y as usize], (RSIZEMAP - x) * 3 ; slot_hv; [0, 1, 2, 3, 4, 6, 7]] as usize,
            _  => slot_cmp![self.value[y as usize], (RSIZEMAP - x) * 3 ; slot_hv; [0, 1, 2, 3, 4, 5, 6, 7, 8]] as usize
        };

        count += match y
        {
            1 => slot_cmp![self.value_rotate[x as usize], y * 3 ; slot_hv; [0, 3, 6]] as usize,
            2 => slot_cmp![self.value_rotate[x as usize], y * 3 ; slot_hv; [0, 1, 3, 4, 6]] as usize,
            3 => slot_cmp![self.value_rotate[x as usize], y * 3 ; slot_hv; [0, 1, 2, 3, 4, 6, 7]] as usize,
            _ => slot_cmp![self.value_rotate[x as usize], y * 3 ; slot_hv; [0, 1, 2, 3, 4, 5, 6, 7, 8]] as usize
        };
       ...
       let conv:(i128, i128) = self.conv_xy((x, y));
        ...
        count += match conv.0
        {
            ...
            _ => slot_cmp![self.value_diagonale[conv.1 as usize], conv.0 * 3 ; slot_d; [0, 1, 2, 3, 4, 5, 6, 7, 8]] as usize
        };
        ...
        count += match (RSIZEMAP as i128) * 2 - conv.1
        {
            ...
            _ => slot_cmp![self.value_diagonale_rotate[conv.0 as usize], ((RSIZEMAP as i128) * 2 - conv.1) * 3 ; slot_d; [0, 1, 2, 3, 4, 5, 6, 7, 8]] as usize
        };
